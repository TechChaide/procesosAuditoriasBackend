...
module.exports = {
  ...
  createOrUpdate: async (req, res) => {
    try {
      ...
      if (codigo_estado == 0 || codigo_estado === undefined) {
        // Create new record
        const newRecord = await Estado.create({
        ...
        const response = await Estado.findByPk(newRecord.codigo_estado);
        ...
        const record = await Estado.findByPk(codigo_estado);
        ...
        // Update fields
        if (nombre_estado !== undefined) record.nombre_estado = nombre_estado;
        ...
        const updatedRecord = await Estado.findByPk(codigo_estado);
  ...
  getAll: async (req, res) => {
    try {
      const rows = await Estado.findAll({
  ...
  getById: async (req, res) => {
    try {
      ...
      const record = await Estado.findByPk(id);
