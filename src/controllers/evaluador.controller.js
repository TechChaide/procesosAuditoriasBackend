...
module.exports = {
  ...
  createOrUpdate: async (req, res) => {
    try {
      ...
      if (codigo_evaluador == 0 || codigo_evaluador === undefined) {
        // Create new record
        const newRecord = await Evaluador.create({
          evaluador: evaluador,
        ...
        const response = await Evaluador.findByPk(newRecord.codigo_evaluador);
        ...
        const record = await Evaluador.findByPk(codigo_evaluador);
        ...
        // Update fields
        if (evaluador !== undefined) record.evaluador = evaluador;
        if (correo_evaluador !== undefined) record.correo_evaluador = correo_evaluador;
        ...
        const updatedRecord = await Evaluador.findByPk(codigo_evaluador);
  ...
  getAll: async (req, res) => {
    try {
      const rows = await Evaluador.findAll({
  ...
  getById: async (req, res) => {
    try {
      ...
      const record = await Evaluador.findByPk(id);
